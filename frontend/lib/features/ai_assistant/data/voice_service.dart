...
class VoiceService {
  ...
  Future<VoiceTranscription> transcribe(Uint8List audioData,
    ...
    final response = await _dio.post('/ai/voice/transcribe',
        data: formData, options: Options(contentType: 'multipart/form-data'));
    return VoiceTranscription.fromJson(response.data);
  }

  Future<VoiceResponse> voiceChat(Uint8List audioData,
      {String sessionId = 'voice-default', String language = 'auto'}) async {
    final formData = FormData.fromMap({
      'file': MultipartFile.fromBytes(audioData, filename: 'audio.wav'),
    ...
    final response = await _dio.post('/ai/voice/respond',
        data: formData, options: Options(contentType: 'multipart/form-data'));
    return VoiceResponse.fromJson(response.data);
  }

  Future<VoiceResponse> voiceChatWithAudio(Uint8List audioData,
      {String sessionId = 'voice-default', String language = 'auto'}) async {
    final formData = FormData.fromMap({
      'file': MultipartFile.fromBytes(audioData, filename: 'audio.wav'),
      'session_id': sessionId,
    ...
    final response = await _dio.post('/ai/voice/chat',
        data: formData, options: Options(contentType: 'multipart/form-data'));
    return VoiceResponse.fromJson(response.data);
  }

  Future<VoiceResponse> textToVoiceChat(String text,
      {String sessionId = 'voice-default'}) async {
  ...
  void connectWebSocket(String sessionId) {
    ...
    _wsChannel =
        WebSocketChannel.connect(Uri.parse('$baseUrl/ai/voice/ws/$sessionId'));
    _wsChannel!.stream.listen(
      ...
      onError: (e) => _eventController.add({
        'type': 'error',
