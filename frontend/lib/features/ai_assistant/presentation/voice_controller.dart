...
class VoiceChatNotifier extends StateNotifier<VoiceChatState> {
  ...
  VoiceChatNotifier(this._voiceService)
      : super(VoiceChatState(
          ...
          messages: [
            AIMessage(
              role: 'assistant',
              content:
                  'أهلاً! أنا مساعدك الصوتي. اضغط على الميكروفون واتكلم عادي.\n\n'
                  'ممكن تسألني:\n'
                  '• "كام في الخزنة؟"\n'
    ...
    switch (type) {
      case 'transcription_complete':
        ...
        state = state.copyWith(
            voiceState: VoiceState.processing, partialTranscription: null);
        break;
  ...
  Future<void> processAudio(Uint8List audioData) async {
    ...
    try {
      ...
      state = state.copyWith(
        voiceState: response.audioBase64 != null
            ? VoiceState.speaking
            : VoiceState.idle,
  ...
  Future<void> sendTextMessage(String text) async {
    ...
    try {
      ...
      _addMessage(
          AIMessage(role: 'assistant', content: 'حصل مشكلة، جرب تاني.'));
      state = state.copyWith(voiceState: VoiceState.idle);
    }
  }
