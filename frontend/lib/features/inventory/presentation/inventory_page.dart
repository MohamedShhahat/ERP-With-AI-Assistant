...
class _InventoryPageState extends ConsumerState<InventoryPage> {
  ...
  void _showAiDialog() {
    showDialog(
        context: context, builder: (ctx) => _InventoryAiDialog(ref: ref));
  }
  ...
  void _showTransferDialog(InventoryItem item) {
    showDialog(
        context: context,
        builder: (_) => _TransferDialog(ref: ref, item: item));
  }

  void _showAdjustStockDialog(InventoryItem item) {
    showDialog(
        context: context,
        builder: (_) => _AdjustStockDialog(ref: ref, item: item));
  }

  void _showStockHistoryDialog(InventoryItem item) {
    showDialog(
        context: context,
        builder: (_) => _StockHistoryDialog(ref: ref, item: item));
  }

  void _showAddStockDialog(InventoryItem item) {
    showDialog(
        context: context,
        builder: (_) => _AddStockDialog(ref: ref, item: item));
  }

  void _showDeductStockDialog(InventoryItem item) {
    showDialog(
        context: context,
        builder: (_) => _DeductStockDialog(ref: ref, item: item));
  }
                ...
                Row(
                  children: [
                    _KpiCard(
                        ...
                        label: '\$${_formatNumber(kpis['totalValue'])}',
                ...
                SingleChildScrollView(
                  ...
                  child: Row(
                    children: [
                      ...
                      _StatusChip(
                          label: 'In Stock',
                          selected: statusFilter == StockStatus.normal,
                          color: AppColors.success,
                          onTap: () => ref
                              .read(inventoryStatusFilterProvider.notifier)
                              .state = StockStatus.normal),
                      const SizedBox(width: 8),
                      _StatusChip(
                          label: 'Low Stock',
                          selected: statusFilter == StockStatus.low,
                          color: AppColors.warning,
                          onTap: () => ref
                              .read(inventoryStatusFilterProvider.notifier)
                      ...
                      _StatusChip(
                          label: 'Out of Stock',
                          selected: statusFilter == StockStatus.outOfStock,
                          color: AppColors.error,
                          onTap: () => ref
                              .read(inventoryStatusFilterProvider.notifier)
                      ...
                      _StatusChip(
                          label: 'Overstock',
                          selected: statusFilter == StockStatus.overstock,
                          color: AppColors.info,
                          onTap: () => ref
                              .read(inventoryStatusFilterProvider.notifier)
                              .state = StockStatus.overstock),
                    ],
                  ),
                ),
                const SizedBox(height: 16),
          ...
          children: [
            ...
            SizedBox(
              ...
              child: Column(
                ...
                children: [
                  Text('${item.totalStock.toStringAsFixed(1)} $unit',
                      style: const TextStyle(
        ...
        child: Column(
          ...
          children: [
            DropdownButtonFormField<int>(
              ...
              items: warehouses
                  .map((w) => DropdownMenuItem(
                      value: w.warehouseId, child: Text(w.warehouseName)))
                  .toList(),
            ...
            TextFormField(
              ...
              decoration: InputDecoration(
                  labelText: 'Quantity (${widget.item.baseUnit})'),
      ...
      actions: [
        ...
        ElevatedButton(
          style: ElevatedButton.styleFrom(
              backgroundColor:
                  _direction == 'out' ? AppColors.error : AppColors.success),
          onPressed: _loading
              ? null
              : () async {
        ...
        child: Column(
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration:
                  const InputDecoration(labelText: 'Cost per unit (EGP)'),
              keyboardType: TextInputType.number,
            ),
            const SizedBox(height: 16),
            TextFormField(
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text(
                      'Current stock: ${widget.item.totalStock.toStringAsFixed(1)} ${widget.item.baseUnit}',
                      style: const TextStyle(
                          fontSize: 13, fontWeight: FontWeight.w500)),
                ],
              ),
            ...
            TextFormField(
              controller: _qtyController,
              decoration: InputDecoration(
                  labelText: 'Quantity to deduct (${widget.item.baseUnit}) *'),
              keyboardType: TextInputType.number,
            ),
            const SizedBox(height: 16),
            TextFormField(
              ...
              decoration: const InputDecoration(
                  labelText: 'Reason (waste, damage, correction) *'),
              maxLines: 2,
            ),
          ],
        ),
              ...
              : () async {
                  ...
                  try {
                    ...
                    if (mounted) {
                      Navigator.pop(context);
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                          content: Text('Stock deducted successfully'),
                          backgroundColor: Colors.green));
                    }
                  } catch (e) {
                    if (mounted)
    ...
    return AlertDialog(
      title: Row(
        children: [
          ...
          const SizedBox(width: 8),
          Expanded(
              child: Text('History: ${widget.item.productName}',
                ...
                : _transactions == null || _transactions!.isEmpty
                    ? const Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: 4),
                            Text('Transactions will appear here once recorded',
                                style:
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          final costPerUnit = double.tryParse(
                                  tx['cost_per_unit']?.toString() ?? '0') ??
                              0;
                          final notes = tx['notes']?.toString() ?? '';
                          final createdDate =
                              tx['created_date']?.toString() ?? '';
                          final displayDate = createdDate.length > 16
                              ? createdDate
                                  .substring(0, 16)
                                  .replaceFirst('T', ' ')
                              : createdDate;
                          ...
                          return ListTile(
                            ...
                            title: Row(
                              children: [
                                ...
                                const Spacer(),
                                Text('${quantity.toStringAsFixed(2)} $unitType',
                                    style: const TextStyle(
                                        fontSize: 13,
                                        fontWeight: FontWeight.w700)),
                              ],
                                ...
                                Row(
                                  children: [
                                    ...
                                    if (costPerUnit > 0) ...[
                                      const Spacer(),
                                      Text(
                                          '@ \$${costPerUnit.toStringAsFixed(2)}/unit',
                                          style: const TextStyle(
                                              fontSize: 11,
                      ...
                      children: [
                        ...
                        Text('No active alerts',
                            style: TextStyle(
                                fontSize: 16, fontWeight: FontWeight.w500)),
                        SizedBox(height: 4),
                        Text('All inventory levels are within normal range',
            ...
            Expanded(
              child: _loading
                  ...
                  : _response != null
                      ...
                      : const Center(
                          child: Text('Ask me anything about your inventory!',
                              style:
                                  TextStyle(color: AppColors.textSecondary))),
            ),
          ],
