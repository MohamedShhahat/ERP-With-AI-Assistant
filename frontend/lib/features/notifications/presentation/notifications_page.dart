...
class NotificationsPage extends ConsumerWidget {
          ...
          Row(
            children: [
              ...
              _FilterChip(
                  ...
                  onTap: () => ref.read(severityFilterProvider.notifier).state =
                      severity == 'critical' ? null : 'critical'),
              const SizedBox(width: 8),
              _FilterChip(
                  label: 'Warning',
              ...
              data: (notifications) {
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (context, index) => _NotificationCard(
                    ...
                    onMarkRead: () =>
                        _markRead(ref, notifications[index].notificationId),
                  ),
                );
              },
            ),
  ...
  Future<void> _runChecks(BuildContext context, WidgetRef ref) async {
    ...
    try {
      ...
      if (context.mounted) {
        final newAlerts =
            result['new_notifications'] as Map<String, dynamic>? ?? {};
        final total = (newAlerts['low_stock_alerts'] as int? ?? 0) +
            (newAlerts['credit_limit_exceeded'] as int? ?? 0) +
            (newAlerts['overdue_supplier_payments'] as int? ?? 0);
        if (total > 0) {
          _showSnackBar(
            context,
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          Text(_formatDate(notification.createdDate),
                              style: TextStyle(
                                  fontSize: 12,
                                  color: isDark
                                      ? AppColors.darkTextSecondary
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
              'Run "Check Alerts" to scan for low stock, credit limits, and overdue payments',
              style: TextStyle(
                  fontSize: 13,
                  color: isDark
                      ? AppColors.darkTextSecondary
