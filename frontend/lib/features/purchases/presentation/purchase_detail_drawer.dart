    ...
    if (_returns.isNotEmpty) {
      tableHtml += buildTableHtml(
        ...
        rows: _returns.asMap().entries.map((entry) {
          ...
          final r = entry.value;
          final rDate =
              r.returnDate != null ? _formatDate(r.returnDate!) : 'N/A';
          return [
            '$i',
            rDate,
  ...
  void _returnItems(PurchaseInvoiceModel inv) {
    if (_items.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('No items loaded yet. Please wait.'),
...
class _PurchaseReturnItemsDialogState
  ...
  void initState() {
    super.initState();
    _qtyControllers =
        widget.items.map((item) => TextEditingController(text: '0')).toList();
  ...
  Future<void> _submit() async {
    ...
    for (int i = 0; i < widget.items.length; i++) {
      ...
      if (qty > widget.items[i].returnableQuantity) {
        setState(() {
          _errorMessage =
              'Cannot return ${qty % 1 == 0 ? qty.toInt() : qty.toStringAsFixed(2)} of "${widget.items[i].productName}" — only ${widget.items[i].returnableQuantity % 1 == 0 ? widget.items[i].returnableQuantity.toInt() : widget.items[i].returnableQuantity.toStringAsFixed(2)} available for return.';
        });
        return;
      }
    ...
    if (returnItems.isEmpty) {
      setState(() => _errorMessage =
          'Please select at least one item and specify a quantity to return.');
      return;
    }
