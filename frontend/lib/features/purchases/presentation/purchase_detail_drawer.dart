...
''';
  ...
  void _returnItems(PurchaseInvoiceModel inv) {
    if (_items.isEmpty) {
      _showStyledSnackBar(context, 'No items loaded yet. Please wait.',
    ...
    if (returnableItems.isEmpty) {
      _showStyledSnackBar(context, 'All items have already been returned',
          type: _SnackType.warning);
      return;
    }
      ...
      builder: (ctx) => _PurchaseReturnItemsDialog(
        ...
        onSubmit: (returnItems, refundAmount, notes) async {
          try {
            ...
            _loadData();
            if (mounted) {
              _showStyledSnackBar(context, 'Return processed successfully',
  ...
  void _showStyledSnackBar(BuildContext ctx, String message,
...
class _PurchaseReturnItemsDialogState
  ...
  Future<void> _submit() async {
    ...
    if (returnItems.isEmpty) {
      _showDialogSnackBar('Select at least one item to return',
          type: _SnackType.warning);
      return;
    }
