        ...
        children: [
          ...
          salesAsync.when(
            data: (data) {
              ...
              return _buildFullWidthTable(isDark,
                  columns: const [
                    DataColumn(
                        label: Text('Date',
                  ...
                  rows: products
                      ...
                      .map<DataRow>((e) => DataRow(cells: [
                            ...
                            DataCell(Text('${e.value['total_quantity']}',
                                style: const TextStyle(fontSize: 13))),
                            DataCell(Text(_fmtAmount(e.value['total_revenue']),
                                style: const TextStyle(
                                    fontSize: 13,
                                    fontWeight: FontWeight.w600))),
                          ]))
...
class _SupplierBalancesTab extends ConsumerWidget {
  ...
  void _print(Map<String, dynamic> suppData) {
    ...
    var tableHtml =
        '<p style="font-size:14px;margin-bottom:10px;"><strong>Total Payables: ${_fmtAmount(suppData['total_payable'])} IQD</strong></p>';
    tableHtml += buildTableHtml(
...
class _CashFlowTab extends ConsumerWidget {
  ...
  void _print(Map<String, dynamic> cashData) {
    ...
    var tableHtml =
        '<p style="font-size:14px;margin:10px 0;"><strong>Summary:</strong> Total In: ${_fmtAmount(flowData['total_in'])} IQD | Total Out: ${_fmtAmount(flowData['total_out'])} IQD | Net: ${_fmtAmount(flowData['net_flow'])} IQD</p>';
    tableHtml += buildTableHtml(
...
Widget _buildFullWidthTable(bool isDark,
