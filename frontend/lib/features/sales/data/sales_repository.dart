...
class InvoiceItemModel {
  ...
  factory InvoiceItemModel.fromJson(Map<String, dynamic> json) {
    return InvoiceItemModel(
      ...
      soldQuantity:
          double.tryParse(json['sold_quantity']?.toString() ?? '0') ?? 0,
      unitType: json['unit_type'] ?? 'meter',
      unitPrice: double.tryParse(json['unit_price']?.toString() ?? '0') ?? 0,
      discount: double.tryParse(json['discount']?.toString() ?? '0') ?? 0,
      totalPrice: double.tryParse(json['total_price']?.toString() ?? '0') ?? 0,
      returnedQuantity:
          double.tryParse(json['returned_quantity']?.toString() ?? '0') ?? 0,
    );
  }
...
class SalesReturnModel {
  ...
  factory SalesReturnModel.fromJson(Map<String, dynamic> json) {
    return SalesReturnModel(
      ...
      returnedAmount:
          double.tryParse(json['returned_amount']?.toString() ?? '0') ?? 0,
      refundAmount:
...
class SalesRepository {
  ...
  Future<SalesInvoiceModel> update(
  ...
  Future<SalesReturnModel> createReturn(int invoiceId,
